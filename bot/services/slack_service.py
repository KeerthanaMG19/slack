...
class SlackService:
    ...
    def get_dm_channel_id(self, user_id):
        ...
        try:
            ...
            logger.error(f"Error opening DM with {user_id}: {str(e)}", exc_info=True)
            return None

    def fetch_unread_dm_messages(self, dm_channel_id, user_id):
        """Fetch unread messages from a DM channel for a user"""
        try:
            from ..models import UserSummaryState
            last_ts = UserSummaryState.get_last_summary_ts(user_id, dm_channel_id)
            messages = []
            cursor = None
            newest_ts = None
            while True:
                response = self.client.conversations_history(channel=dm_channel_id, oldest=str(last_ts), cursor=cursor, limit=200)
                for msg in response.get('messages', []):
                ...
                time.sleep(self.RATE_LIMIT_DELAY)
            if newest_ts and messages:
                UserSummaryState.update_last_summary_ts(user_id, dm_channel_id, newest_ts)
            return sorted(messages, key=lambda x: float(x['ts']))
        except Exception as e:
            logger.error(f"Error fetching unread DM messages: {str(e)}", exc_info=True)
            return []
